...
| Apps Script | Next.js |
|---|---|
| `SpreadsheetApp.getActiveSpreadsheet()` / `openByUrl()` | `googleapis` Sheets API v4, authenticated as a service account (`src/lib/sheets.ts`) |
...
3. Share the **Master Control** spreadsheet (SETTINGS/ASSETS/GRAPHICS/BOWLS)
   with the service account's email as a Viewer. Share whatever spreadsheet
   `SETTINGS!CURRENT_SHEET_LINK` currently points at the same way — and
   re-share every time you switch to a new week's spreadsheet.
4. Copy `.env.example` to `.env.local` and fill in `MASTER_SPREADSHEET_ID`,
